...
Print bytecode generated by the Ignition interpreter to the standard output.

Code in:
* `src/interpreter/interpreter.cc:ShouldPrintBytecode()`
* `src/interpreter/interpreter.cc:InterpreterCompilationJob::FinalizeJobImpl()`

### `--print-bytecode-filter`

type: `string`, default: `*`

Filter for selecting which functions to print bytecode for (used when `--print-bytecode` is enabled). The syntax of filter is:
...
Code in:
* `src/interpreter/interpreter.cc:ShouldPrintBytecode()`
* `src/objects/objects.cc:SharedFunctionInfo::PassesFilter()`
* `src/utils/utils.cc:PassesFilter`
